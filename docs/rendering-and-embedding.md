...
**DOM rendering coverage (#9).** Unlike the pure modules above, `renderer.js` reads `document`/`window` off the global scope, so exercising its actual DOM-construction code (not just the type-registry key list) needs a real-enough DOM. `tests/run-tests.mjs` §13 installs a single `jsdom` window as globals (with small stubs for the browser APIs jsdom doesn't implement — `matchMedia`, `scrollIntoView`, `requestAnimationFrame`, `speechSynthesis`) and then, for every embedded docs example and `demo-worksheet.json`: calls `renderWorksheet` end-to-end and asserts a non-empty tree with no activity falling back to the broken-render placeholder, and calls every `RENDERERS[type]` directly. It also regression-tests translation-compare's `ResizeObserver`-based layout (not a leaked `window` resize listener — see the paragraph above, issue #7).
